...
export default function CheckoutPage() {
	...
	async function purchaseCart(){
		...
		try{
			...
			await axios.post(import.meta.env.VITE_API_URL + "/api/orders",{address : address,
	...
	return (<div className="w-full lg:h-[calc(100vh-100px)] overflow-y-scroll bg-primary flex flex-col pt-[25px] items-center">
			<div className="w-[400px] lg:w-[600px] flex flex-col gap-4 ">
				{cart.map((item, index) => {
					return (
						<div
							key={index}className="w-full h-[300px] lg:h-[120px] bg-white flex flex-col lg:flex-row relative items-center p-3 lg:p-0"
						>
							<button
							...
							<img className="h-[100px] lg:h-full aspect-square object-cover"
								src={item.image}
							/><div className="w-full text-center lg:w-[200px] h-[100px] lg:h-full flex flex-col pl-[5px] pt-[10px] ">
								...
								{item.labelledPrice > item.price && (<span className="text-secondary lg:w-full   text-center  lg:text-right line-through text-lg pr-[10px] lg:mt-[20px]">
				...
				<div className="w-full lg:w-full h-[120px] bg-white flex flex-col-reverse  lg:flex-row justify-end items-center relative">
					<button
						to="/checkout"
						onClick={purchaseCart}className="lg:absolute left-0 bg-accent text-white px-6 py-3  lg:ml-[20px] hover:bg-accent/80"
					>
						Order
					</button>
